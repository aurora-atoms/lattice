...
def run_case(case_dir: Path) -> tuple[bool, str]:
    ...
    actual_waste = set()
    for line in waste_result.stdout.splitlines():
        record = json.loads(line)
        actual_waste.add(record["payload"]["pattern"])
    ...
    if expected.get("expect_dossier"):
        ...
        if expected_dossier.exists() and out_md.exists():
            if expected_dossier.read_text(encoding="utf-8").strip() != out_md.read_text(encoding="utf-8").strip():
                ok = False
