...
## Workflow
1. Query ConPort before loading or searching the full skill text when ConPort is available; otherwise inspect targeted source files first.
2. Classify content sensitivity.
3. Choose runtime: online Excalidraw for low-sensitivity work, Docker Desktop/self-hosted for local confidential workflows, local development repo for app customization.
4. Create or update artifacts. Use `scripts/create_excalidraw_canvas_project.sh` for a repeatable folder wrapper.
5. Create the story map before drawing. Use `references/story-canvas.md`.
6. Give setup commands only after choosing runtime. Use `references/local-docker.md`.
7. Run security review before sharing. Use `references/security.md`.
...
- Avoid embedding private screenshots or full source dumps. Use labels, bounded summaries, placeholders, or separately reviewed image assets.
- Validate generated files with `python3 -m json.tool <file>` and a lightweight structural check before returning.

## Rules
EXC.001 | MUST | preserve | preserve existing canvas behavior, layout intent, and source boundaries | enforce
EXC.002 | MUST | privacy | classify sensitivity before selecting an online or local runtime | enforce
EXC.003 | MUST | quality | optimize quality-adjusted token ROI instead of blind token reduction | enforce
EXC.004 | SHOULD | prompt | keep stable canvas rules and output contracts in a stable prefix | prefer
EXC.005 | NEVER | data | include secrets, unrestricted logs, or regulated private content | block
EXC.006 | NEVER | edit | replace an existing layout wholesale when a scoped repair was requested | block

## Verification
Before returning:

- Confirm the canvas is local-first and does not contain secrets, logs, credentials, or unrestricted private dumps.
...
- Embedding screenshots, secrets, credentials, logs, or private source dumps.
