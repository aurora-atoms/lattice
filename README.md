...
For a bounded consequential action, use the [Safety-Critical Product Review reference workflow](docs/safety-critical-product-review.md) to evaluate and improve the requirement before tracing it through the invariant, enforcement point, runtime evidence, adversarial test, failure classification, and release gate.

The workflow reuses the Senior Attention profile and existing capabilities rather than adding a safety mega Skill or Agent. Its validator can recommend `block`, `conditional`, or `pass_candidate`; it cannot certify a product or approve architecture, safety, compliance, deployment, or release. Real product evidence and decisions remain private downstream.
